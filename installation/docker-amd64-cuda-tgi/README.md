...
8. Push your generic image (the one with the root user) to some registry if not done already.
   This will be handy for you, for sharing it with your teammates, and when you open-source your project later.
    ...
    3. Add this link to the TODO ADD PULL_IMAGE_NAME in
       ...
       e.g., ic-registry.epfl.ch/your-lab/your-gaspar/packing_.)
...
./template.sh run python -m packing.template_experiment some_arg=some_value
...
```bash
mv packing packing-tmp
mkdir packing
mv packing-tmp packing/dev
...
E.g., you can mount it at `/project/packing` and specify `PROJECT_ROOT_AT=/project/packing`.
...
export PROJECT_ROOT_AT=/project/packing
apptainer run \
    -c \
    ...
    --env WANDB_API_KEY="" \
    --nv packing_amd64-cuda-root-latest.sif

...
runai submit \
  ...
  --image registry.rcp.epfl.ch/claire/moalla/packing:amd64-cuda-moalla-latest \
  --pvc runai-claire-moalla-scratch:/claire-rcp-scratch \
  --working-dir /claire-rcp-scratch/home/moalla/packing/run \
  -e PROJECT_ROOT_AT=/claire-rcp-scratch/home/moalla/packing/run \
  -g 1 --cpu 8 --cpu-limit 8 --memory 64G --memory-limit 64G --large-shm \
  -- python -m packing.template_experiment some_arg=2 wandb.mode=offline

# template_experiment is an actual script that you can run.
# or -- zsh packing/reproducibility-scripts/template-experiment.sh
...
# That would be a copy of the packing repo frozen in a commit at a working state to be used in unattended jobs.

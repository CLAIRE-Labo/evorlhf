...
export PROJECT_ROOT_AT=$SCRATCH/packing/run
export SLURM_ONE_ENTRYPOINT_SCRIPT_PER_NODE=1
export WANDB_API_KEY_FILE_AT=$HOME/.wandb-api-key

srun \
  --container-image=$CONTAINER_IMAGES/claire+moalla+packing+amd64-cuda-root-latest.sqsh \
  --container-mounts=\
/etc/slurm,\
$SCRATCH,\
$WANDB_API_KEY_FILE_AT \
  ...
  bash -c 'sleep 60; python -m packing.template_experiment some_arg=LOCALID-$SLURM_LOCALID-PROCID-$SLURM_PROCID'
